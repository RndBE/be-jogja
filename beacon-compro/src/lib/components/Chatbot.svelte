<script lang="ts">
	...
	async function pollMessages() {
		...
		try {
			...
			const res = await fetch(`${PUBLIC_API_BASE}/chatbot/poll?${params}`, {
	...
	function validateForm(): boolean {
		...
		if (!formOrganization.trim()) errors.organization = 'Instansi wajib diisi';
		if (!formPhone.trim()) errors.phone = 'Nomor telepon wajib diisi';
		else if (formPhone.trim().length < 8) errors.phone = 'Nomor telepon tidak valid';
		formErrors = errors;
	...
	async function submitContactForm() {
		...
		try {
			...
			if (res.ok) {
				...
				formErrors = { general: data.error || 'Gagal mengirim data. Coba lagi.' };
			}
		} catch {
	...
	function getModeLabel(): string {
		switch (chatMode) {
			case 'escalated': return 'Menghubungkan ke CS...';
			case 'live': return 'Terhubung ke CS';
			case 'closed': return 'Sesi berakhir';
			default: return isLoading ? 'Mengetik...' : 'Online';
		}
	}

	function getModeColor(): string {
		switch (chatMode) {
			case 'escalated': return 'bg-amber-500 animate-pulse';
...
>
		...
		<div class="flex items-center gap-3">
			<div class="relative w-10 h-10 rounded-xl flex items-center justify-center bg-zinc-900 border border-white/10 shadow-inner">
				...
				<div class="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-zinc-950 flex items-center justify-center">
					<div class="w-1.5 h-1.5 rounded-full {getModeColor()}"></div>
				</div>
			</div>
			<div>
				...
				<span class="text-[10px] font-mono font-semibold uppercase tracking-widest {isLiveMode() ? 'text-emerald-500' : 'text-zinc-500'}">
					{getModeLabel()}
				</span>
			</div>
		</div>
	...
	<!-- Chat Area -->
	<div class="relative z-10 p-5 h-[360px] overflow-y-auto flex flex-col gap-4 bg-zinc-950/80 scroll-smooth" bind:this={chatAreaRef}>
		{#each messages as msg}
			{#if msg.role === 'assistant'}
				<div class="flex flex-col gap-1 items-start max-w-[85%]">
					{#if msg.senderType === 'agent'}
						<span class="text-[10px] font-mono font-bold text-emerald-500 uppercase tracking-widest pl-1 mb-0.5">
							{msg.senderName || 'CS Agent'}
						</span>
					{/if}
					<div class="p-3.5 rounded-2xl rounded-tl-sm border text-sm leading-relaxed shadow-sm whitespace-pre-wrap
						{msg.senderType === 'agent' ? 'bg-emerald-950/50 border-emerald-800/30 text-emerald-200' : 'bg-zinc-900 border-white/5 text-zinc-300'}">
						{msg.content}
					</div>
					<span class="text-[10px] font-mono text-zinc-600 pl-1">{msg.timestamp}</span>
				</div>
			{:else}
				<div class="flex flex-col gap-1 items-end max-w-[85%] self-end">
					<div class="p-3.5 rounded-2xl rounded-tr-sm text-sm text-white leading-relaxed shadow-sm whitespace-pre-wrap" style="background: linear-gradient(135deg, #C8102E 0%, #910B20 100%);">
						{msg.content}
					</div>
					<span class="text-[10px] font-mono text-zinc-600 pr-1">{msg.timestamp}</span>
				</div>
			{/if}
		{/each}
		...
		{#if isLoading}
			<div class="flex flex-col gap-1 items-start max-w-[85%]">
				<div class="p-3.5 rounded-2xl rounded-tl-sm bg-zinc-900 border border-white/5 shadow-sm flex items-center gap-2">
					<Loader size={14} class="text-[#C8102E] animate-spin" />
					<span class="text-xs text-zinc-500 font-medium">
						{isLiveMode() ? 'Mengirim pesan...' : 'Beacon sedang mengetik...'}
					</span>
				</div>
			</div>
		{/if}
		...
		{#if showContactForm}
			<div class="w-full">
				<div class="rounded-2xl border border-white/10 bg-zinc-900 p-4 space-y-3">
					<div class="flex items-center gap-2 mb-1">
						<UserCheck size={14} class="text-[#C8102E]" />
						<span class="text-xs font-bold text-white uppercase tracking-wider">Data Kontak</span>
					</div>
					...
					{#if formErrors.general}
						<div class="px-3 py-2 rounded-lg bg-red-950/50 border border-red-800/30 text-xs text-red-400">
							{formErrors.general}
						</div>
					{/if}
					...
					<!-- Name -->
					<div class="space-y-1">
						<label class="text-[10px] font-mono font-bold text-zinc-500 uppercase tracking-widest flex items-center gap-1.5">
							<User size={10} />
							Nama Lengkap
						</label>
						<input
						...
						/>
						{#if formErrors.name}
							<span class="text-[10px] text-red-400">{formErrors.name}</span>
						{/if}
					</div>
					...
					<div class="space-y-1">
						...
						{#if formErrors.organization}
							<span class="text-[10px] text-red-400">{formErrors.organization}</span>
						{/if}
					</div>
					...
					<div class="space-y-1">
						...
						/>
						{#if formErrors.phone}
							<span class="text-[10px] text-red-400">{formErrors.phone}</span>
						{/if}
					</div>
		...
		{#if chatMode === 'escalated' && !showContactForm}
			<div class="flex items-center gap-2 px-4 py-3 rounded-xl bg-amber-950/30 border border-amber-800/30">
				<Loader size={14} class="text-amber-500 animate-spin" />
				<span class="text-xs text-amber-400 font-medium">Menunggu CS tersedia...</span>
			</div>
		{/if}
		...
		{#if showSuggestions}
			<div class="flex flex-col gap-2 mt-2">
				...
				{#each predefinedQuestions as q}
					...
					>
						{q}
						<span class="float-right opacity-0 group-hover:opacity-100 text-[#C8102E] transition-opacity">
							<Send size={12} />
						</span>
					</button>
				{/each}
	...
	<div class="relative z-10 p-4 border-t border-white/5 bg-zinc-950">
		...
		{:else if showContactForm}
			<div class="text-center py-2">
				<span class="text-xs text-zinc-500">Lengkapi form di atas untuk terhubung dengan CS</span>
			</div>
		{:else}
			<form onsubmit={(e) => { e.preventDefault(); sendMessage(); }} class="relative group">
				...
				<div class="relative flex items-center bg-zinc-900 border border-white/10 rounded-xl overflow-hidden shadow-inner">
					...
					<input
						type="text"
						bind:value={message}
						placeholder={isLoading ? 'Menunggu...' : (isLiveMode() ? 'Ketik pesan ke CS...' : 'Ketik pertanyaan...')}
						disabled={isLoading}
					...
					<button
						...
						class="p-2 mr-1 rounded-lg transition-colors {message.trim() && !isLoading ? 'bg-[#C8102E] text-white hover:bg-[#910B20]' : 'bg-transparent text-zinc-600'}"
					>
						<Send size={14} />
					</button>
				</div>
		...
		<div class="text-center mt-3">
			<span class="text-[9px] font-mono uppercase tracking-[0.2em] text-zinc-600">Powered by Beacon AI</span>
		</div>
	</div>
</div>
