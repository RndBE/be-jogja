<script lang="ts">
	...
	onMount(() => {
		...
		const handleScroll = () => {
			...
			scrollProgress = Math.min(100, (scrolled / (total - window.innerHeight)) * 100);
		};
...
<section class="relative py-16 lg:py-24 overflow-hidden" style="background: linear-gradient(165deg, #FFFFFF 0%, #FFF5F6 40%, #FBE9EC 100%);">
	...
	<div class="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
		<!-- Back link -->
		<a href="/wawasan"
			class="inline-flex items-center gap-2 text-sm font-medium mb-8 transition-all hover:text-[#C8102E] hover:gap-3 btn-tactile"
...
>
	<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
		<div class="prose-beacon space-y-8">
			...
			<!-- Tags -->
			{#if tags.length > 0}
				<div class="flex flex-wrap items-center gap-2 pt-6" style="border-top: 1px solid #E5E5E5;">
		...
		<div class="grid lg:grid-cols-[1.4fr_1fr] gap-10 items-center">
			...
			<div class="flex flex-col sm:flex-row gap-3">
				<a href={ctaWhatsApp} target="_blank" rel="noopener"
					class="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl text-sm font-semibold text-white btn-tactile"
				...
				<a href={ctaSecondary.href}
					class="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl text-sm font-semibold btn-tactile"
	...
	<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
		...
		<div class="grid md:grid-cols-2 gap-4">
			{#each relatedArticles as related}
				<a href={related.href} class="group p-5 rounded-[16px] transition-all duration-300 hover:-translate-y-0.5 btn-tactile" style="border: 1px solid #E5E5E5; background: white;">
